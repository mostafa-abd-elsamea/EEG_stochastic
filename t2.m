...
%to,t1,t2 
global iteration;
global kkk;global snr_t2;global run;
xi2=0;
xi6=1;
...
dataset1 = readmatrix(['C:\Users\Mustafa\Desktop\my_thesis_4oct_2022\mat1s\data_txt\c_fc\',sprintf('%s',kkk),'\',sprintf('%s',run) , '/c4t2_d.txt']);
dataset2 = readmatrix(['C:\Users\Mustafa\Desktop\my_thesis_4oct_2022\mat1s\data_txt\c_fc\',sprintf('%s',kkk),'\',sprintf('%s',run) , '/c4t2_d_f_abs.txt']);
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%final brain data%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5error sum%%%%%%%%%%%%%%%%%%%%%%5
for j=1:n
    sin_fn_c3t0(j)=(xi6*sin((2*pi*z(2)*delta*t(j))+z(3)));
end
%amp_stocastic for c3 and t0
amp_stocastic_c3t0(1)=z(5);%data_cal(1);%-0.205136407;
for i=1:n-1
    amp_stocastic_c3t0(i+1)=amp_stocastic_c3t0(i)+z(1)*(xi2-amp_stocastic_c3t0(i))*delta+z(4)*dw(i+1);
end
...
f3=sum(err_f_c3);
snr_t2(iteration)=10*log10(sum(f_spc_c3(1:5))/f3);
end