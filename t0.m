...
%if you want to switch between channels only just change the 2 datasets\
%\text file
%to,t1,t2 
global iteration;
global kkk;global snr_t0;global run;
xi2=0;
xi6=1;
Fs=160;%sample frequency;
%itration=5;%change here for itterations  c3t0_bp_d
%%
%reading the txt file contains raw c3c4f7f8
dataset1 = readmatrix(['C:\Users\Mustafa\Desktop\my_thesis_4oct_2022\mat1s\data_txt\c_fc\',sprintf('%s',kkk) ,'\',sprintf('%s',run) ,'/c4t0_d.txt']);
dataset2 = readmatrix(['C:\Users\Mustafa\Desktop\my_thesis_4oct_2022\mat1s\data_txt\c_fc\',sprintf('%s',kkk) ,'\',sprintf('%s',run) ,'/c4t0_d_f_abs.txt']);
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%final brain data%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
amp_stocastic_c3t0(1)=x(5);%data_cal(1);%-0.205136407;
for i=1:n-1
...
f1=sum(err_f_c3);
snr_t0(iteration)=10*log10(sum(f_spc_c3(1:5))/f1);
%plot (data_cal_abs(1:5));%here******************************8
%hold on;
%plot (f_spc_c3(1:5));
%hold off;
%plot(data_cal_abs);hold on;plot(f_spc_c3);
end